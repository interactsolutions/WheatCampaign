(() => {
  ...
  // Read JSON that is embedded in HTML to avoid fetch/CORS problems in some environments.
...
let active = 0;
  ...
  async function loadCampaign(id) {
    ...
    // Optional media config (prefer embedded JSON to avoid fetch/CORS issues)
    state.mediaCfg = readEmbeddedJson('wc-media-config');
    if (!state.mediaCfg) {
      try {
