// dashboard.js - COMPLETE FIXED VERSION v4.0 - Media Integration Fixed
console.log('AgriVista Dashboard v4.0 - Complete Fixed Media Integration initializing...');
...
document.addEventListener('DOMContentLoaded', function () {
    console.log('DOM loaded. Starting initialization with complete media integration...');
...
async function loadAllData() {
    try {
        ...
        updateStatus('Dashboard loaded successfully with media integration', 'success');
...
// LOAD MEDIA DATA WITH FIXED JSON PARSING
async function loadMediaData() {
    try {
        ...
        if (response.ok) {
            ...
            // Handle the array structure of your JSON
            if (Array.isArray(data) && data.length > 0) {
                // Your JSON is an array with one object containing mediaItems
                mediaItems = data[0].mediaItems || [];
            } else if (data.mediaItems) {
                // Direct object with mediaItems property
                mediaItems = data.mediaItems;
            } else if (Array.isArray(data)) {
                // JSON is directly an array of media items
                mediaItems = data;
            } else {
                mediaItems = [];
            }
            ...
            // Ensure all media items have proper paths
            mediaItems = mediaItems.map(item => {
                // If filename doesn't start with gallery/, add it
...
function updateMediaCounts() {
    ...
    // Update all media count elements
...
// CREATE MEDIA FROM STRUCTURE (FALLBACK)
...
function safeRenderGallery() {
    try {
        ...
        const galleryMedia = mediaItems.filter(item => {
            ...
            // Fallback: include all items for now
            return true;
        });
...
console.log('AgriVista Dashboard v4.0 Complete Fixed Version with Media Integration loaded successfully.');
